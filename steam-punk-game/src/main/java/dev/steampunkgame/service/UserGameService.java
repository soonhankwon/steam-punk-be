...
public class UserGameService {
    ...
    @Transactional
    public List<UserGameAddResponse> addUserGame(UserGameAddRequest request) {
        List<UserGameHistory> userGameHistories = new ArrayList<>();
        request.productIds().forEach(id -> {
            if (userGameHistoryRepository.existsByUserIdAndProductId(request.userId(), id)) {
        ...
        return userGameHistories.stream()
                .map(UserGameAddResponse::from)
